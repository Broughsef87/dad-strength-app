...
| `assets/*.svg` | `src/components/Logo.tsx`, `scripts/generate-logo-suite.mjs` → `public/logo-suite/` | the plate stack, adopted — see Brand marks |
...
**Adopted, 2026-09-05.** The DS proposed a plate-stack mark — three bars of
decreasing width in brand-ink on a volt field, `assets/ds-mark-volt.svg` — and
Andrew took it in place of the DS monogram. Three things carry it and the check
reads the DS file's rects and asserts all three agree:
...
rotated, gradiented or placed on a category tint (readme). Minimum 20px.
