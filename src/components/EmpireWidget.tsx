'use client'

import { useState, useEffect } from 'react'
import { TrendingUp, DollarSign, Youtube, MonitorSmartphone, ArrowRight, Target, Activity } from 'lucide-react'
import Link from 'next/link'
import { createClient } from '../utils/supabase/client'
import { useUser } from '../contexts/UserContext'
import { motion } from 'framer-motion'
import { fadeUp } from './ui/motion'

const ICON_MAP = {
  trending: TrendingUp,
  dollar: DollarSign,
  youtube: Youtube,
  saas: MonitorSmartphone,
  target: Target,
  activity: Activity
}

export default function EmpireWidget() {
  const supabase = createClient()
  const { user } = useUser()
  const [mission, setMission] = useState({
    title: 'The Empire',
    primaryMetric: 'Operation: Freedom',
    current: 125000,
    target: 1000000,
    unit: '$',
    secondary1Label: 'Subs',
    secondary1Value: '12,450',
    secondary1Icon: 'youtube',
    secondary2Label: 'MRR',
    secondary2Value: '$2,100',
    secondary2Icon: 'saas'
  })

  useEffect(() => {
    async function loadMission() {
      // Try Supabase first
      try {
        if (user) {
          const { data: profile } = await supabase
            .from('user_profiles')
            .select('mission_data')
            .eq('id', user.id)
            .single()

          if (profile?.mission_data) {
            setMission(profile.mission_data as unknown as typeof mission)
            return
          }
        }
      } catch (err) {
        console.error('Error loading mission from Supabase:', err)
      }

    }
    loadMission()
  }, [user])

  const percentage = Math.round((mission.current / mission.target) * 100)
  const isCurrency = mission.unit === '$'
  const displayCurrent = isCurrency ? `${(mission.current / 1000).toFixed(0)}K` : mission.current.toLocaleString()
  const displayTarget = isCurrency ? `${(mission.target / 1000000).toFixed(0)}M` : mission.target.toLocaleString()

  const S1Icon = ICON_MAP[mission.secondary1Icon as keyof typeof ICON_MAP] || TrendingUp
  const S2Icon = ICON_MAP[mission.secondary2Icon as keyof typeof ICON_MAP] || MonitorSmartphone

  return (
    <motion.div
      variants={fadeUp}
      initial="hidden"
      animate="visible"
      className="space-y-4"
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <TrendingUp className="w-5 h-5 text-brand" strokeWidth={1.5} />
          <h3 className="font-display font-semibold text-foreground lowercase italic">{mission.title}</h3>
        </div>
        <Link href="/profile/edit-mission" className="text-muted-foreground hover:text-foreground transition-colors p-2 bg-background rounded-lg border border-border">
          <ArrowRight size={14} strokeWidth={1.5} />
        </Link>
      </div>

      {/* Primary Target Progress */}
      <div className="bg-background/50 p-4 rounded-2xl border border-border shadow-inner">
        <div className="flex justify-between items-end mb-3">
          <div>
            <p className="text-xs font-display font-semibold lowercase text-muted-foreground mb-1">{mission.primaryMetric}</p>
            <p className="font-display font-semibold text-xl text-foreground tracking-wide tabular-nums">
              {isCurrency && '$'}{displayCurrent}
              <span className="text-muted-foreground text-xs ml-1 font-bold">/ {isCurrency && '$'}{displayTarget} {mission.unit !== '$' && mission.unit}</span>
            </p>
          </div>
          <span className={`text-xs font-display font-semibold tabular-nums ${percentage > 100 ? 'text-green-500' : 'text-brand'}`}>{percentage}%</span>
        </div>
        <div className="h-2 w-full bg-card rounded-full overflow-hidden border border-border p-[1px]">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${Math.min(percentage, 100)}%` }}
            transition={{ duration: 1, ease: [0.25, 0.46, 0.45, 0.94], delay: 0.3 }}
            className={`h-full rounded-full ${percentage > 100 ? 'bg-green-500' : 'bg-brand'}`}
          />
        </div>
      </div>

      {/* Secondary Stats */}
      <div className="grid grid-cols-2 gap-3">
        <motion.div
          whileHover={{ y: -1 }}
          className="bg-background/50 p-3 rounded-xl border border-border flex items-center gap-3 group hover:border-brand/30 transition-all"
        >
          <div className="p-2 bg-card rounded-lg">
             <S1Icon size={16} strokeWidth={1.5} className="text-brand group-hover:text-brand/80 transition-colors" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-display font-semibold text-muted-foreground lowercase truncate">{mission.secondary1Label}</p>
            <p className="text-xs font-display font-semibold text-foreground tracking-wide tabular-nums">{mission.secondary1Value}</p>
          </div>
        </motion.div>
        <motion.div
          whileHover={{ y: -1 }}
          className="bg-background/50 p-3 rounded-xl border border-border flex items-center gap-3 group hover:border-brand/30 transition-all"
        >
          <div className="p-2 bg-card rounded-lg">
             <S2Icon size={16} strokeWidth={1.5} className="text-brand group-hover:text-brand/80 transition-colors" />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-display font-semibold text-muted-foreground lowercase truncate">{mission.secondary2Label}</p>
            <p className="text-xs font-display font-semibold text-foreground tracking-wide tabular-nums">{mission.secondary2Value}</p>
          </div>
        </motion.div>
      </div>
    </motion.div>
  )
}
