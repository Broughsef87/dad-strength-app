...
export default function StreakShield() {
  ...
  return (
    <div className="tile rounded-xl p-5 space-y-4">
      ...
      {/* State: active today */}
      {activeToday && (
        <div className="flex items-center gap-3 bg-status-good-fill/10 border border-status-good-fill/20 rounded-xl px-4 py-3">
          <ShieldCheck size={22} className="text-status-good-ink shrink-0" />
          <div>
            <p className="text-sm font-semibold text-status-good-ink">Streak Protected</p>
      ...
      {!usedThisMonth && !activeToday && (
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-brand text-brand-ink text-sm font-semibold tracking-wide hover:opacity-90 active:scale-[0.97] transition-all"
          >
            <Shield size={15} />
            Activate Shield
          </button>
