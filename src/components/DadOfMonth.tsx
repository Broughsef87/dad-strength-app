...
function getGrade(pct: number): { label: string; color: string } {
  ...
  if (pct >= 75) return { label: 'Strong', color: 'text-foreground' }
  if (pct >= 60) return { label: 'Solid', color: 'text-foreground' }
  if (pct >= 40) return { label: 'Grinding', color: 'text-brand' }
  return { label: 'Rise Up', color: 'text-destructive' }
}
  ...
  return (
    <div className="tile rounded-xl p-5 space-y-4">
      ...
      <div className="flex items-center gap-1.5">
        <span
          className={`text-xs font-semibold ${delta >= 0 ? 'text-foreground' : 'text-destructive'}`}
        >
