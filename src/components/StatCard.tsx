  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[9px] font-medium lowercase text-brand">Forge OS / Archive</span>
          </div>
          <h2 className="text-4xl font-light text-background leading-none mb-1 tracking-tight">
            {title}
          </h2>
          <p className="text-sm font-light lowercase text-background/50">{subtitle}</p>
        </div>

        <div className="grid grid-cols-1 gap-5 relative z-10">
          {stats.map((stat, i) => (
            <div key={i} className="border-l border-background/20 pl-4">
              <p className="text-[9px] font-medium lowercase text-background/50 mb-1">{stat.label}</p>
              <div className="flex items-baseline gap-2">
                <p className="text-3xl font-light text-background tabular-nums">{stat.value}</p>
                {stat.subtext && <p className="text-xs font-medium text-brand lowercase">{stat.subtext}</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-between items-end relative z-10">
          <div className="text-[8px] font-medium lowercase text-background/30">
      ...
      <div className="flex gap-2">
        <button
          onClick={handleShare}
          className="flex-1 bg-foreground hover:opacity-90 text-background font-medium lowercase py-3.5 text-xs flex items-center justify-center gap-2 transition-opacity active:scale-[0.98] rounded-lg"
        >
