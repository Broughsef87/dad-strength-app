    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display font-semibold text-foreground lowercase italic text-lg">Family Pulse</h3>
        </div>
        <div className="flex items-center gap-2">
          {saving && (
            <span className="text-[9px] font-display font-semibold lowercase text-gray-600 animate-pulse">Saving…</span>
          )}
          <span className="text-[9px] font-display font-semibold lowercase text-muted-foreground">{weekLabel}</span>
        </div>
      </div>
      ...
      <div className="bg-background/80 rounded-2xl border border-border p-5 flex items-center gap-5">
        <div className="flex-1">
          <p className="text-[9px] font-display font-semibold lowercase text-muted-foreground mb-1">Pulse Score</p>
          ...
          {!hasAnyData && (
            <p className="text-[9px] text-brand font-display font-semibold lowercase mt-1">Rate below to start</p>
          )}
        </div>
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          {lastWeekScore !== null && (
            <span className="text-[9px] font-display font-semibold lowercase text-gray-600">
              Last: {lastWeekScore.toFixed(1)}
            </span>
          )}
          {lastWeekScore === null && (
            <span className="text-[9px] font-display font-semibold lowercase text-gray-700">No prev</span>
          )}
        </div>
      </div>
      ...
      <div className="bg-card/40 rounded-2xl border border-border p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-display font-semibold lowercase text-muted-foreground">Marriage Vibe</p>
          <span className="text-[9px] font-display font-semibold text-rose-400 lowercase">
      ...
      <div className="bg-card/40 rounded-2xl border border-border p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-display font-semibold lowercase text-muted-foreground">Kid Connection</p>
          <span className="text-[9px] font-display font-semibold text-brand lowercase">
      ...
      <div className="bg-card/40 rounded-2xl border border-border p-4 space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-display font-semibold lowercase text-muted-foreground">Intentional Moments</p>
          <span className="text-[9px] font-display font-semibold text-muted-foreground lowercase">
        ...
        <div className="space-y-2">
          {current.moments.map((m, i) => (
            <div key={i} className="flex items-center gap-2 p-2 rounded-xl bg-background/60 border border-border group animate-in fade-in slide-in-from-left-1">
              <span className="text-[9px] font-display font-semibold text-brand lowercase w-4 shrink-0">{i + 1}</span>
      ...
      {/* Tagline */}
      <p className="text-[10px] text-center text-gray-600 lowercase font-display font-semibold tracking-widest italic">
