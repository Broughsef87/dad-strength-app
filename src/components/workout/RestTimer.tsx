...
export default function RestTimer({ timeLeft, onSkip }: RestTimerProps) {
  ...
  return (
    <div className={`relative overflow-hidden p-5 rounded-3xl border-2 flex items-center justify-between transition-all duration-500 ${
      isLowTime
        ? 'bg-status-danger-fill/10 border-status-danger-fill/50 shadow-[0_0_20px_rgba(239,68,68,0.1)]'
      ...
      <div
        className={`absolute bottom-0 left-0 h-1 transition-all duration-1000 ease-linear ${
          isLowTime ? 'bg-status-danger-fill' : 'bg-brand'
        }`}
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div>
          <p className="text-[10px] font-black text-muted-foreground lowercase mb-1 flex items-center gap-2">
            Protocol Rest
            {isLowTime && <span className="inline-block w-1.5 h-1.5 rounded-full bg-status-danger-fill" />}
          </p>
          <p className={`text-3xl font-mono font-black tabular-nums leading-none ${isLowTime ? 'text-status-danger-ink' : 'text-foreground'}`}>
            {formatTime(timeLeft)}
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-2 px-1">
          ...
          <X className="w-4 h-4 text-gray-600 group-hover:text-status-danger-ink transition-colors" />
        </div>
      </button>
    </div>
  );
