...
export default function ExerciseCard({
 ...
 isCompleted
 ? 'border-status-good-fill/40 bg-status-good-fill/[0.02] shadow-[0_15px_40px_-10px_rgba(16,185,129,0.1)]'
 ...
 isCompleted ? 'bg-status-good-fill/[0.03]' : ''
 }`}>
          <div className="flex-1 min-w-0 pr-2">
            <div className="flex items-center gap-2 mb-2.5 flex-wrap">
              <h3 className={`font-display font-semibold text-2xl tracking-wide transition-all duration-500 ${
 isCompleted ? 'text-status-good-ink' : 'text-foreground'
 }`}>
                {name.toUpperCase()}
              </h3>
              {isCompleted && (
                <div className="bg-status-good-fill/20 p-1.5 rounded-full animate-in zoom-in duration-500">
                  <CheckCircle2 size={18} className="text-status-good-ink" strokeWidth={3} />
                </div>
              )}
            </div>
 ...
 isCompleted
 ? 'bg-status-good-fill/10 border-status-good-fill/20 text-status-good-ink hover:text-status-good-ink'
...
function MenuItem({
 ...
 destructive ? 'text-status-danger-ink hover:bg-status-danger-fill/5' : className
 }`}
    >
