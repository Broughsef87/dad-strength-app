  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-background/95 backdrop-blur-xl animate-in fade-in duration-500 overflow-y-auto">
      <div className="w-full max-w-sm relative my-8 animate-in zoom-in-95 slide-in-from-bottom-10 duration-700 ease-out">
        ...
        <div className="bg-card border border-indigo-500/20 rounded-[48px] p-8 text-center shadow-[0_0_50px_rgba(99,102,241,0.2)] relative overflow-hidden">
          {/* Top Accent */}
          ...
          <div className="relative z-10">
            ...
            <div className="mb-10 space-y-4">
              <div className="bg-gray-800/50 border border-gray-700 rounded-3xl p-6 transform hover:scale-[1.02] transition-transform">
                ...
                <p className="text-4xl font-black text-foreground">{totalVolume.toLocaleString()}<span className="text-sm text-indigo-500 ml-1 italic">LBS</span></p>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-800/50 border border-gray-700 rounded-3xl p-4">
                  <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1">Duration</p>
                  <p className="text-xl font-black text-foreground font-mono">{duration}</p>
                </div>
                <div className="bg-gray-800/50 border border-gray-700 rounded-3xl p-4">
                  <p className="text-[8px] font-black text-muted-foreground uppercase tracking-widest mb-1">Rank</p>
            ...
            <div className="space-y-4 pt-6 border-t border-border">
              <button 
                onClick={onReturn}
                className="group w-full bg-indigo-600 text-foreground font-black py-5 rounded-3xl hover:bg-indigo-500 transition-all active:scale-95 flex items-center justify-center gap-3 shadow-[0_10px_20px_rgba(99,102,241,0.3)]"
              >
                RETURN TO HQ
                <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform" />
              </button>
              
              <button className="w-full flex items-center justify-center gap-2 text-muted-foreground hover:text-foreground transition-colors py-2 text-[10px] font-black uppercase tracking-widest">
                <Share2 size={14} />
