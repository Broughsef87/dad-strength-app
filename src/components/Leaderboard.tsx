  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold flex items-center gap-2 text-foreground lowercase italic">
          <Trophy className="w-5 h-5 text-yellow-500" />
          The Feed
        </h2>
        <span className="text-[10px] font-display font-semibold bg-brand/10 text-brand px-3 py-1 rounded-full lowercase border border-brand/20">
          Global
        </span>
      </div>
        ...
        {MOCK_PRS.map((pr) => (
          <div key={pr.id} className="flex items-start gap-4 p-4 rounded-xl border border-border bg-background/50 hover:bg-gray-800 transition-all group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-2">
                <span className="inline-flex items-center gap-1 text-[10px] font-display font-semibold px-2 py-1 rounded-md bg-card border border-border text-brand lowercase">
      ...
      <button className="w-full pt-2 text-[10px] font-display font-semibold text-gray-600 hover:text-muted-foreground transition-colors flex items-center justify-center gap-2 lowercase">
        View Rankings
      </button>
    </div>
  );
