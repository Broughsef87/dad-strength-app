      ...
      <main className="max-w-md mx-auto p-6 pb-28 space-y-8">
        ...
        <section className="space-y-3">
          ...
          {notifPermission === 'denied' && (
            <p className="text-xs text-status-danger-ink px-1">
              Notifications blocked by browser. Enable in browser settings to receive reminders.
            </p>
          )}
          {notifPermission === 'granted' && (
            <p className="text-xs text-status-good-ink px-1">
              ...
              <div className="p-4 space-y-3 bg-muted/30">
                {resetSent ? (
                  <div className="flex items-center gap-2 text-sm text-foreground">
                    <Check size={16} className="text-status-good-ink" />
                  ...
                  <>
                    ...
                    {resetError && (
                      <p className="text-xs text-status-danger-ink">{resetError}</p>
                    )}
                    <button
        ...
        {(hasBilling || (isPro && !isFounder)) && (
          <section className="space-y-3">
            ...
            {portalError && <p className="text-xs text-status-danger-ink px-1">{portalError}</p>}
            <p className="text-xs text-muted-foreground px-1">
        ...
        <section className="space-y-3">
          ...
          <div className="bg-card rounded-xl border border-status-danger-fill/20 overflow-hidden">
            <button
              onClick={() => setShowDeleteForm(!showDeleteForm)}
              className="w-full flex items-center gap-4 p-4 hover:bg-status-danger-fill/5 transition-colors"
            >
              <div className="p-2 bg-status-danger-fill/10 rounded-lg">
                <Trash2 size={16} className="text-status-danger-ink" />
              </div>
              <div className="text-left flex-1">
                <p className="font-medium text-sm text-status-danger-ink">Delete Account</p>
                <p className="text-xs text-muted-foreground">
            ...
            {showDeleteForm && (
              <div className="p-4 space-y-3 border-t border-status-danger-fill/20 bg-status-danger-fill/[0.02]">
                ...
                <input
                  ...
                  placeholder="delete"
                  className="w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-status-danger-fill/50"
                  autoComplete="off"
                />
                {deleteError && <p className="text-xs text-status-danger-ink">{deleteError}</p>}
                <button
